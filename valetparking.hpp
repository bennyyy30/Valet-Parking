...
class ValetParking {
  public:
    ...
    size_t getNextTicket();  // return the next ticket# to be issued to customer
    size_t checkIn();  // on success: return stall# (index-1 base), on failure: return 0
    size_t stallNumber(size_t ticket);  // return index-1 base stall# (stalls are numbered 1, 2, 3, ...n)
  ...
  private:
    ...
    size_t _currentticket;  // the current ticket# issued to customer
    double _fee;  // the valet parking fee
    double _paid;  // the total transaction amount so far from paid cars exiting the lot
